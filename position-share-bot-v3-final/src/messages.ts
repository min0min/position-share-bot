import type { Account, HistoryItem, Position } from './bitget/client.js';
...
function marginRatio(p: Position, equity: number) { return equity > 0 ? (p.marginSize / equity) * 100 : 0; }
function pnlRate(p: Position) { return p.marginSize > 0 ? (p.unrealizedPL / p.marginSize) * 100 : 0; }
function emojiForClose(pnl: number) { return pnl >= 0 ? '🎉 익절' : '🛑 손절'; }
...
export function status(account: Account, positions: PositionState[]): string {
  ...
  if (positions.length === 0) return [...header, '', '현재 보유 포지션 없음'].join('\n');
