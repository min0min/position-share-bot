...
export function n(value: unknown, digits = 4): string {
...
export function gauge(percent: number, blocks = 10): string {
...
export function pnlIcon(v: number): string {
  if (v > 0) return '🟢';
  if (v < 0) return '🔴';
  return '⚪';
}

export function sideIcon(side: string): string {
...
export function holdTime(ms: number): string {
