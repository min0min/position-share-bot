...
import { addEvent, closeEvent, equity as equityMsg, help, history as historyMsg, online, openEvent, period, reduceEvent, status } from './messages.js';
...
async function handleCommand(cmd) {
    ...
    if (cmd === '/status' || cmd === '/positions') {
        await safeSend(status(account, [...store.positions.values()]));
    }
    else if (cmd === '/equity') {
...
async function main() {
    console.log('Position Share Bot v3.0 starting...');
    await scanPositions();
    while (true) {
