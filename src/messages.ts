import type { Account, HistoryItem } from './bitget/client.js';
import type { ClosedTrade, PositionState } from './state/store.js';
import { gauge, holdTime, money, n, pct, pnlIcon, sideIcon } from './utils/format.js';

function marginRatio(p: PositionState, equity: number) { return equity > 0 ? (p.marginSize / equity) * 100 : 0; }
function pnlRate(p: PositionState) { return p.marginSize > 0 ? (p.unrealizedPL / p.marginSize) * 100 : 0; }
function closeTitle(pnl: number) { return pnl >= 0 ? '🎉 익절' : '🛑 손절'; }
function shortSymbol(symbol: string) { return symbol.replace(/USDT$/i, ''); }

export function online(account: Account, count: number): string {
  return [
...
export function equity(account: Account): string {
  return [
    '💰 계좌 상태',
    '',
    `Equity: ${n(account.equity, 2)} USDT`,
...
export function statusSummary(account: Account, positions: PositionState[]): string {
...
export function positionCard(p: PositionState, equity: number): string {
...
export function openEvent(p: PositionState, equity: number): string {
  return [
...
export function addEvent(prev: PositionState, cur: PositionState, equity: number): string {
...
export function reduceEvent(prev: PositionState, cur: PositionState, equity: number): string {
...
export function closeEvent(prev: PositionState, realizedEstimate: number): string {
  return [
...
export function period(title: string, items: HistoryItem[] | ClosedTrade[], equity: number): string {
  const profits = items.map((x: any) => Number(x.netProfit ?? x.realized ?? x.pnl ?? 0));
...
export function history(items: HistoryItem[] | ClosedTrade[]): string {
  const rows = items.slice(0, 10).map((x: any, i) => {
