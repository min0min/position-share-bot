...
import { duration, fmt, gauge, pct, pnlEmoji, pnlLabel, riskLabel, sideEmoji, sideLabel, sign } from './utils/format.js';
...
function fullSymbol(symbol: string): string {
  return symbol.includes('USDT') ? symbol : `${symbol}USDT`;
}

function direction(p: PositionSnapshot): string {
  return `${sideEmoji(p.side)} ${sideLabel(p.side)}`;
}

function pnlBlock(value: number, roi?: number, title = '손익'): string {
...
function positionCard(p: PositionSnapshot, equity: number): string {
...
export function statusMsg(positions: PositionSnapshot[], equity: number): string {
  if (!positions.length) return `${LINE}\n📭 현재 오픈 포지션 없음\n${LINE}\n\n💰 Equity\n${fmt(equity)} USDT`;
